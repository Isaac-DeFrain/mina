  ...
  let gcloud_keyfile =
    ...
    | Some keyfile ->
        if upload_blocks_to_gcloud then
          [%log info]
            "GCLOUD_KEYFILE environment variable set to %s" keyfile ;
        Some keyfile
    | _ ->
        if upload_blocks_to_gcloud then
          [%log warn]
                ...
                | Some network ->
                    ...
                    [%log info]
                      ~metadata:
                        [ ("block", `String name)
                        ; ("path", `String path)
                        ; ("time", `String Time.(now () |> to_string_iso8601_basic ~zone:Zone.utc))
                        ]
                      "Logged precomputed $block to $path at $time"
                | None -> () )
           else
             [%log info]
               ~metadata:[ ("state_hash", `String (State_hash.to_base58_check hash)) ]
